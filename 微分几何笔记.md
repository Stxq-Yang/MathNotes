...
~~服了，我感觉要补习一下分析，点集直接跳过来学微几跨度太大了,虽然还能理解~~  
## *What is 流形?*  
...
第二可数意味着这个空间是可分的，Hausdorff性质意味着点不会全部粘在一起，这都非常自然，允许了度量的存在。坐标卡的存在，允许了围绕任意一点的附近区域都能展开到欧几里得空间下，精准的描述了局部的欧几里得性质，让每个点周围的区域都能像欧几里得空间一样处理，或者反过来说，我们用欧几里得空间进行同胚粘合出了一个新的空间。  
...
具体的，我们定义为满足$dx^i \frac{\partial}{\partial x^i}\big |_p=1$且$dx^i \frac{\partial}{\partial x^j}\big |_p=0(i\ne j)$的$dx^i$为它的基，成立显然。
...
0 &(i\ne j)
\end{cases}$$
于是$\forall i,j \in [1,n]\cap \mathbb{N},dx^i\frac{\partial}{\partial x^j}\big |_p=\delta^i_j $即成立，$\{dx^i\}_{i,j \in [1,n]\cap \mathbb{N}}$为余切空间$T^*_pM$的基。

## 余切丛
化归了那么多欧氏空间的性质，现在我们又要来化归自己了。想必任何人看到这个名词都已经能自然而然地写下这段定义了吧

*定义*
$$T^*M=\{(p,v)|p\in M,v\in T^8_pM\}$$

然后我们就要证明它也是个流形，这个很简单，我们不难发现其实做了个对偶和$TM$是拓扑同胚的。因为这个映射$v\mapsto \alpha$可以拆成$\frac{\partial}{\partial x^i}\big |_p\mapsto dx^i$是显然线性同构的，合起来就是拓扑同胚的，所以很有趣，它依旧是$2 \operatorname{dim} M$维的光滑流形。
