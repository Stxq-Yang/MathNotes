...
我们知道$\mathbb{R}^n\times \mathbb{R}^n\cong \mathbb{R}^{2n}$,可以赋予一个标准拓扑$\tau_{\mathbb{R}^{2n}}$。我们就干脆把它射回去，即走$\widetilde{\varphi}^{-1}:\mathbb{R}^{2n}\to \pi^{-1}(U)$，我们保证是一个连续映射，那么我们就得到了一个新的拓扑，嗯好，接下来我们要用这个局部的子空间拓扑制造全局的$TM$的拓扑。
