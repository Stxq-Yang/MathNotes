...
*不妨定义为伪函子$H[v](g)=v(g\circ \psi^{-1} \circ \varphi)$。由于$g$可以是任意的$U\cap V$上的光滑函数，那么在这个范围内，$H[T_{\varphi^{-1}(r^1,\cdots,r^n)}M]=T_{\psi(\varphi^{-1}(r^1,\cdots,r^n))}M$，在之前我们已经证明其可以表示为$\sum_{i\in [1,n]\cap\mathbb{Z}} w^i\frac{\partial}{\partial x^i} \big |_{\psi(\varphi^{-1}(r^1,\cdots,r^n))},w^i\in \mathbb{R}$。故可以认为映射$(v^1,\cdots,v^n)\mapsto (w^1,\cdots,w^n)$同样是光滑的。*
